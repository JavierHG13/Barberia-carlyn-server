...
- **RandomForestRegressor**: modelo no lineal; captura patrones por semana, ocupacion y ticket promedio.
"""),
    code(COMMON_SETUP),
    md("""
...
"""),
    md("""
## 4. Modelo principal: RandomForestRegressor

RandomForestRegressor combina muchos arboles para capturar relaciones no lineales: por ejemplo, semanas con mayor demanda, tickets mas altos y ocupacion.
"""),
    code("""
...
pd.DataFrame([linear_metrics, rf_metrics], index=["Regresion lineal", "RandomForestRegressor"])
"""),
    md("""
...
Esta libreta construye una segmentacion de clientes con **KMeans**.
...
KMeans usa distancias. Si no escalamos, `gasto_total` dominaria al resto solo por tener numeros mas grandes.
"""),
    code("""
...
Entrenamos KMeans con varios valores de K. Buscamos el punto donde agregar mas clusters ya no reduce tanto la inercia.
"""),
    code("""
ks = range(2, 9)
inertias = []
...
El dendrograma no entrena KMeans, pero ayuda a entender que tan separados estan los grupos. Para que sea legible, se toma una muestra de clientes.
"""),
    code("""
...
KMeans solo entrega numeros de cluster. Nosotros asignamos nombres de negocio interpretando los promedios.
"""),
    code("""
labels = {}
used = set()
...
Guardamos el escalador, el modelo KMeans y las etiquetas de negocio.
"""),
    code("""
...
def main():
    write_notebook("01_clasificacion_inasistencias.ipynb", "Clasificacion: prediccion de inasistencias", classification_cells)
    write_notebook("02_pronostico_ingresos.ipynb", "Regresion: pronostico de ingresos semanales", regression_cells)
    write_notebook("03_segmentacion_clientes.ipynb", "Clustering: segmentacion de clientes", clustering_cells)
